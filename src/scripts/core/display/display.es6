	...
	update_infobox( evt ) {
		if ( evt ) {
			...
			if ( evt.EVENT.runstats ) {
				e = document.getElementById("ui-runstats");
				...
				if ( evt.EVENT.runid && evt.EVENT.evtid ) {
					e = document.getElementById("ui-runstats-run-evt");
					if ( e ) {
						e.innerHTML = evt.EVENT.runid + ' / ' + evt.EVENT.evtid;
					}
				}
				if ( evt.EVENT.timeStr ) {
					e = document.getElementById("ui-runstats-date");
					if ( e ) {
						e.innerHTML = evt.EVENT.timeStr;
					}
				} else if ( evt.EVENT.time ) {
					e = document.getElementById("ui-runstats-date");
					if ( e ) {
						e.innerHTML = moment.unix( evt.EVENT.time ).format('YYYY-MM-DD HH:mm:ss ZZ');
					}
				}
				if ( evt.EVENT.type && evt.EVENT['s_nn'] ) {
					e = document.getElementById("ui-runstats-collisions");
					if ( e ) {
						e.innerHTML = evt.EVENT.type + ' @ √' + evt.EVENT['s_nn'] + ' GeV';
					}
				}
			}
			e = document.getElementById("ui-infobox");
			if ( e ) {
				e.style.visibility = 'visible';
			}
		} else {
			let e = document.getElementById("ui-infobox");
			if ( e ) {
