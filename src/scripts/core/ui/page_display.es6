...
export default class UIPageDisplay {
	...
	async get_github_events() {
		if ( !window.octokit ) { return []; }
		const {
			data: { login },
		} = await window.octokit.rest.users.getAuthenticated();
		if ( !login ) { return []; }

		let contents = await window.octokit.rest.repos.getContent({
			owner: 'sPHENIX-Collaboration',
			repo: 'sPHENIXDisplayEventCache',
			path: '.'
		});
		if ( !contents || !contents.data || !contents.data.length ) {
			return [];
		}
		let files = [], name, link;
		for( let i = 0, ilen = contents.data.length; i < ilen; i++ ) {
			name = contents.data[i].name;
			link = contents.data[i].download_url;
	...
	async get_github_html() {
		let data = await this.get_github_events();
		let html = '';
		for ( let i = 0, ilen = data.length; i < ilen; i++ ) {
			html += '<option value="' + data[i].link + '">' + data[i].name + '</option>' + "\n";
		}
		return html;
	}
	...
	async init() {
		...
		let github_events = await this.get_github_html();

        $('body').append(`
            <div data-role="page" id="edisplay" style="background-color: black; overflow: hidden;">
                ...
                <div id="viewer-header" data-role="header" data-position="fixed" data-fullscreen="true" data-tap-toggle="true">
                ...
                <div role="main" class="ui-content">
					...
					<div id="ui-infobox" style="visibility: hidden;">
						<div id="epic-logo"></div>
						<div id="ui-runstats" style="font-family: inherit !important;">
							<div style="font-family: inherit !important;">ePIC Experiment at EIC</div>
							<div style="font-family: inherit !important;">Data recorded: <div id="ui-runstats-date">N/A</div></div>
							<div style="font-family: inherit !important;">Run / Event: <div id="ui-runstats-run-evt">N/A</div></div>
							<div style="font-family: inherit !important;">Collisions: <div id="ui-runstats-collisions">N/A</div></div>
						</div>
					</div>
                </div>
                <div id="viewer-footer" data-role="footer" data-position="fixed" data-fullscreen="true" data-tap-toggle="true">
				...
				<div data-role="popup" id="event-popup" data-overlay-theme="b" data-theme="b" data-dismissible="false" style="min-width: 500px; max-width:700px;">
					...
					...OR...
					<label for="event-collection-github">GitHub Events</label>
					<select name="github-event-list" id="github-event-list">
						<option value="">- not selected -</option>
						${github_events}
					</select>
				...
				<div data-role="popup" id="camera-popup" data-overlay-theme="b" data-theme="b" data-dismissible="false" style="min-width: 1200px; max-width:1200px;">
				    ...
				    <div role="main" id="camera-popup-main" style="padding: 10px; text-align: center;">
						...
						<fieldset data-role="controlgroup" data-type="horizontal" id="camera-background">
					    	...
					    	<input name="camera-background" id="camera-background-d" value="#002375" type="radio">
						    <label for="camera-background-d">Blue</label>
						</fieldset>
							</td><td width="33%">
		...
		$('#event-popup-apply').on('click', async () => {
			let file_remote = $('#event-remote').val(),
				...
				github_list_val = $('#github-event-list').val(),
			...
			} else if ( github_list_val ) {
				// this is the remote url
				try {
					this.progressOpen();
					this.progressSetValue( 'loading event', 0 );
					await this.display.load_event_as_file( github_list_val );
					this.display.add_event();
					await this.display.add_detector();
					this.progressClose();
					this.display.render();
				} catch(e) {
					alert('ERROR: event file cannot be decoded');
				}
